...
public class EntropyPixelFeature<T extends RealType<T>>
	...
	public void initialize() {
		...
		for (int i = 32; i <= 256; i *= 2) {
			...
			mapOps.add(Computers.unary(ops(), Map.class, IterableInterval.class, in(), shape, filterOp));
	...
	public RandomAccessibleInterval<T> calculate(RandomAccessibleInterval<T> input) {
		...
		for (UnaryFunctionOp<RandomAccessibleInterval<T>, RandomAccessibleInterval<T>> gaussOp : gaussOps) {
			...
			// cursor.next();
			// TODO min, max value
			// BinMapper1d<T> binMapper = new Real1dBinMapper<>(0, 255,
			// currentBinSize, false);
